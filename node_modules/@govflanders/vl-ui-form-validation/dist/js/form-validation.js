(function (global, factory) {
  ...
  var _buildFormvalidationConfig = function _buildFormvalidationConfig(obj) {
    var errorMessage = {
  ...
  var _showSuccessForInput = function _showSuccessForInput(input, errors) {
    if (!errors) {
      ...
      if (input.getAttribute(fvValidationType) === 'multi-select' || input.getAttribute(fvValidationType) === 'select') {
        if (vl.util.exists(input.closest("[".concat(fvSuccessClass, "]")))) {
          vl.util.removeClass(input.closest("[".concat(fvErrorParentAtt, "]")), input.getAttribute(fvErrorClassAtt));
          vl.util.addClass(input.closest("[".concat(fvErrorParentAtt, "]")), input.getAttribute(fvSuccessClass));
        }
      }
    }
  ...
  var _showErrorsForInput = function _showErrorsForInput(input, errors) {
    var errorMessage = input.form.querySelector("[".concat(fvErrorIDAtt, "=\"").concat(input.getAttribute(fvErrorPlchAtt), "\"]"));
      ...
      vl.util.each(errors, function (error) {
        ...
        if (input.getAttribute(fvValidationType) === 'multi-select' || input.getAttribute(fvValidationType) === 'select') {
          if (vl.util.exists(input.closest("[".concat(fvErrorParentAtt, "]")))) {
            vl.util.removeClass(input.closest("[".concat(fvErrorParentAtt, "]")), input.getAttribute(fvSuccessClass));
            vl.util.addClass(input.closest("[".concat(fvErrorParentAtt, "]")), input.getAttribute(fvErrorClassAtt));
          }
        }
  ...
  var _resetInput = function _resetInput(input) {
    var errorPlaceholder = input.form.querySelector("[".concat(fvErrorIDAtt, "=\"").concat(input.getAttribute(fvErrorPlchAtt), "\"]"));
  ...
  function () {
    ...
    _createClass(FormValidation, [{
      ...
      value: function dress(form) {
        ...
        fields = form.querySelectorAll("input, textarea, select, .".concat(fvSelectClass));
        vl.util.each(fields, function (field) {
          ...
          if (validationFormType !== 'submit') {
              ...
              if (event.keyCode === 9) {
                if (event.target) {
                  ...
                  if (vl.util.exists(eventTargetSelect)) {
                    _validateField(eventTargetSelect.querySelector('select'), validationConfig, form);
                  } else {
            ...
            field.addEventListener('change', vl.util.debounce(function (event) {
              _validateField(event.target, validationConfig, form);
            }, 100));
          }
        });
      ...
      value: function dressAll() {
        ...
        var forms = document.querySelectorAll("[".concat(fvValidateAtt, "]:not([").concat(fvDressedAtt, "]):not([data-").concat(vl.ns, "js-dress=\"false\"])"));
