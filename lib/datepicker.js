  ...
  var datePickerclass = "js-".concat(vl.ns, "datepicker"),
    ...
    _createClass(Datepicker, [{
      ...
      value: function dress(datepicker) {
        ...
        if (!datepickerInput.hasAttribute('data-input')) {
          datepickerInput.setAttribute('data-input', true);
        }

        if (!datepickerButton.hasAttribute('data-toggle')) {
          datepickerButton.setAttribute('data-toggle', true);
        }

        var fltpckr = flatpickr(datepicker, settings);
        ...
        var isCurrentDate = function isCurrentDate(value) {
          var newValue = value.split(fltpckr.l10n.rangeSeparator).join('');
          var currentValue = fltpckr.selectedDates.map(function (date) {
            return fltpckr.formatDate(date, datepickerDateFormat);
          }).join('');
          return newValue === currentValue;
        };

        datepickerInput.addEventListener('change', function (e) {
          if (e.target.value && (!fltpckr.latestSelectedDateObj || !isCurrentDate(e.target.value))) {
            fltpckr.setDate(e.target.value, true, datepickerDateFormat);
          }
        });
      }
      /**
